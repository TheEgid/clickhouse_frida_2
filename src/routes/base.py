...
@router.post("/add")
def add_document(doc: AddDoc, api_key: str = Depends(get_api_key_dependency)):
    """Добавление документа (с разделением на чанки и векторизацией)."""
    try:
        ...
        rows = [(chunk, embedding) for chunk, embedding in zip(chunks, embeddings)]

        logger.info(f"Загрузка {len(rows)} строк в ClickHouse...")
        ch.insert("rag_docs", rows, column_names=["text", "embedding"])
...
@router.post("/query")
def query_document(q: Query, api_key: str = Depends(get_api_key_dependency)):
    """Выполнение RAG-запроса (векторный поиск + генерация ответа LLM)."""
    try:
        # Проверка api_key выполнена
        ...
        # Передаем уже проверенный api_key
        ...
        # Используем CosineDistance. ORDER BY ... ASC (меньше = лучше/ближе)
        search_query = f"""
        SELECT
            text,
        ...
        results = ch.query(search_query, parameters={"query_vec": query_embedding})
        ...
        return {
            "answer": answer,
            "sources": [text[:200] for text in retrieved_docs_text],
            "model_used": "frida",
        }
